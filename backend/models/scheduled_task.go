...
type ScheduledTask struct {
	...
	ScriptVariables   map[string]string `gorm:"type:jsonb;serializer:json" json:"script_variables,omitempty"` // 脚本变量
...
type TaskExecution struct {
	ID        string    `gorm:"primaryKey;size:128" json:"id"`
	TaskID    string    `gorm:"size:128;index" json:"task_id"`   // 关联的定时任务 ID
	TaskName  string    `gorm:"size:255;index" json:"task_name"` // 任务名称（冗余）
	...
	ResultData map[string]interface{} `gorm:"type:jsonb;serializer:json" json:"result_data,omitempty"` // 执行结果数据
