...
type ScriptExecution struct {
	ID           string    `gorm:"primaryKey;size:128" json:"id"`   // 执行记录 ID
	ScriptID     string    `gorm:"index;size:128" json:"script_id"` // 关联的脚本 ID
	ScriptName   string    `gorm:"size:255" json:"script_name"`     // 脚本名称（冗余，方便查询）
	InstanceID   string    `gorm:"size:128" json:"instance_id"`     // 浏览器实例 ID
	...
	// 抓取数据
	ExtractedData map[string]interface{} `gorm:"type:jsonb;serializer:json" json:"extracted_data,omitempty"` // 抓取到的数据
