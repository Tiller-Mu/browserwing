...
### P4.7.6 录制生命周期收束（已完成；覆盖此前 P4.7 录制会话条款的冲突部分）
...
验证：
P4.7.6 时序回归（含 HTTP Stop 经 Coordinator `recording_stopped` event 收口、同 operation 重放不二次 runtime Stop）、`go test ./api ./services/browser -count=1`、`go test ./... -count=1`、`pnpm run test:p45-contract`、`pnpm run type-check`、`pnpm run build` 与 `git diff --check` 均已通过。阶段已标记完成；尚未执行数据库部署迁移或 Git 提交/合并。
