...
### P4.7.6 录制生命周期收束（修复中；覆盖此前 P4.7 录制会话条款的冲突部分）

契约：
`RecordingLifecycleService` 是 Start、Sync、Stop、Save、Capture、Cancel 的唯一业务编排入口。每个动作必须携带全局 UUID `operation_id`；`RecordingOperation` 的 `pending|completed|failed` 与规范化 `request_payload_hash` 决定重试：相同请求重放终态，不同请求返回 `operation_id_payload_conflict`，不同 ID 未取得同一 pending runtime effect 时返回 `recording_operation_in_progress` 且不创建失败回执。HTTP handler 仅做参数/错误码映射；Manager 仅持有 runtime lease、receipt 和登录态快照，不读取 RecordingSession 数据库状态；Recorder 仅采集。

会话状态固定为 `starting|recording|stopped|saved|cancelled|failed`。Start 先在同一事务建立 pending operation、`starting` 会话、runtime target identity 与 `base_page_flow_revision`，再启动 runtime，最后原子推进为 `recording`。`RecordingSession` 的持久化变更必须通过 status + `lifecycle_revision` CAS；Sync 另使用严格单调的 `sync_revision`。Cancel 可以收口 `starting`，并同时将原 Start operation 置为可重放的 `failed(start_cancelled)`；`saved` 不能 Cancel。

`TestPage.page_flow_revision` 是 Save 的页面主流程基线。Save 在事务外运行最终 `RecordingNormalizer`，事务内锁定页面/会话并复核草稿身份和 flow 基线；同基线才替换当前 PageScript 并递增页面 revision。不同会话并发保存同一页面时，后到者返回 `page_script_replaced_conflict`；saved 会话的新 Save 只有当前脚本仍来自该会话时才返回脚本，否则 `page_script_superseded`。重新录制删除旧 PageScript 不得删除或改变 TestCase/既有生成输入。

Stop 与 Capture 都必须先持久化 pending operation，运行时 receipt 只在数据库业务事务成功后 ACK。HTTP Stop 不得直接调用 `RecordingLifecycleService.Stop`：它先经 Coordinator 驱动 scoped runtime Stop，再用同一 `recording_stopped` event 分支收口；Coordinator 是生产路径唯一提交 `RecordingSession.stopped` 的入口，页面 driver、HTTP、恢复共用该路径。pending Stop/Capture 的恢复只有 `RecoverPendingOperation` 一条实现路径，供相同 operation 重试或服务启动恢复器调用：Stop 仅能在 `IsRecoverableRecordingDraft` 的结构完整条件下收口；Capture 的未发布快照在业务后端重启后以 `auth_snapshot_unavailable` 终态失败且不改变会话。Save 与 Capture 可并存；Capture 与 Cancel 用会话行锁确定提交顺序；Capture 还必须先锁定稳定的 `project_id + version_id` 登录态作用域，并以 partial unique index 保证每个版本最多一个 `active` ProjectAuthState。登录态资产通过 `ProjectAuthState.source_recording_session_id` 和全局唯一、真实的 `source_snapshot_receipt_id` 自证来源；缺失 Manager receipt 身份必须终态 `runtime_receipt_unavailable`，不得伪造 fallback，也不在会话写资产指针。

Manager registry 保留每个会话最新 `draft_sync` 与 final receipt，Coordinator 以会话串行、持续退避的方式重驱动 `runtime → LifecycleService → DB → ACK`，不因短暂数据库故障丢弃页面内 Stop。Recorder 只交接受限语义 DOMSnapshot（URL、标题、定位线索；无 input value、Cookie、Storage 或原始 HTML）；最终 Stop、runtime event 与持久化 PageScript 使用同一快照。final receipt 15 分钟 TTL 后保留最小 tombstone，由 LifecycleService 以可恢复草稿收口为 `stopped` 或以结构化失败收口为 `failed`。更高 revision 的 final receipt 若在内部 Sync/Normalizer 形成已提交的语义失败，或同 revision 但 payload 与已持久化草稿冲突时，HTTP Stop 与 Coordinator 必须调用同一失败收口：session 与 Stop operation 原子标记 failed，final/auth receipt release，runtime effect 解除占用；数据库、锁或 CAS 基础设施失败则保持 pending/claim 并 Retry。Cancel 或最终 Normalizer 拒绝时，pending Stop operation、session 与 final/auth receipt 必须一起进入确定终态。

Start pending operation 另有 runtime driver fencing：CAS 生成 `runtime_driver_token` 并递增 claim generation；Manager reservation 与 Start 事务 2 必须匹配同一 token/generation。reservation 的 `reserved|cancelling|running` 是单一页面副作用 gate：DB claim 每 10 秒续租；更高 generation 遇到 `reserved` 仅取消旧 context 并等待其 `done`，不能并行打开页面、恢复登录态或启动 Recorder；遇到 `running` 只重绑定 fence 并复用既有 recorder。相同 Start operation 重试若初读发现 DB fence 已过期，LifecycleService 只能在恢复事务锁住最新 operation 后、确认同一 token/generation 仍过期时，才将 session/operation 以 `runtime_lease_lost` 原子收口并在提交后释放完整 runtime scope；若 heartbeat 已续租或 fence 已被接管，返回 `recording_operation_in_progress` 与 `Retry-After`，不得改变 session/operation 或释放 runtime。启动恢复遇到仍有效的 Start fence 必须保留 pending；Coordinator 每秒只查询 lease 已到期的 pending Start 并重走该持锁复核，不能把 Stop/Capture 纳入轮询。不得用新 generation 越过旧 reservation。旧 driver 的晚到结果或 release 只能处理自己仍持有的 reservation，绝不推进 `starting → recording` 或停止已被新 fence 接管的 recorder。已完成/失败 Start 的重放必须先按 operation identity 返回，再读取可变登录态；pending Start 仅使用创建时固定的 `SourceAuthStateID`。浏览器实例关闭必须先在 Manager 锁内提取 scope/最新 draft 并创建 `runtime_lease_lost` tombstone，再清理 lease、Recorder 和 receipt；Coordinator 优先复用已有 pending Stop 收口，只有不存在 pending Stop 时才创建不占 runtime effect 的确定性内部 Stop；已终态会话只 ACK tombstone，不创建额外 Stop operation。前端 ledger 的 Start key 固定为 `project_id + version_id + page_id + recording_kind`；`auth_context`、`browser_instance_id` 与 `runtime_page_id` 是冻结 payload，pending 期间变化必须本地拒绝而非创建并行 Start。终态清理只依赖显式稳定业务码表，未知码与基础设施错误继续复用 operation ID；页面管理不再提供无 `recording_session_id` 的直接 Capture，登录态只能从已停止的 `login_flow` 会话发布。

草稿与 runtime event 另有三项不可变协议：`draft_sync` 仅在 revision 严格增大时写入；`recording_stopped` 的 revision 小于或等于当前会话时，receipt 只能证明停止，绝不覆盖 ActionTrace、DOMSnapshot、RecordingMeta、draft hash 或 sync revision。该 stale receipt 仅可经 Normalizer 和 `source_receipt_id + artifact_fingerprint` 去重后补充独立下载产物。Coordinator 对每个 runtime event 返回显式 `Ack|Release|Retry`：只有业务事务已经提交才 ACK/Release；数据库、锁、CAS 可重试和上下文错误一律 Retry，保留 Manager event/receipt。scope mismatch 必须先把会话和 pending Start/Stop 持久化收口为 `failed(runtime_receipt_scope_mismatch)`，再 Release；已终态会话 tombstone 直接 ACK。

Save 在锁外保留 Normalizer candidate 及 draft identity；事务内若发现 Stop 已把初读 `recording` 变为 `stopped`、或 lifecycle revision/draft hash 已变化，必须退出锁并重新 Normalize，绝不以零值 candidate 写 PageScript。前端 ledger 以 `prepare(key, canonicalPayload)` 冻结深拷贝请求：相同请求使用原 operation ID 和原 payload；输入变化本地返回 `recording_operation_input_changed`，不发 HTTP、不创建新 UUID、不 settle 原 entry。

本轮进一步固定四项 runtime 协议：可选 scoped lookup 必须由 `projectAuthRuntimeHolder` 动态转发到底层 Manager adapter；final receipt 使用共享的 `RecordingDraftFingerprintV1`，仅 ActionTrace 或去除根级 `captured_at` 后的语义 DOM 变化才递增 `sync_revision`；Manager 的 event ACK/Release 仅消费匹配 event，只有 LifecycleService 已持久化不兼容终态才调用 session-level runtime cleanup；页面内 Stop 轮询只依赖 Recorder context 与 `activeByInstance` 的 scoped lease，不读取 legacy 全局 `activePage`。Start ledger key 固定为 `project_id + version_id + page_id + recording_kind`，运行时身份保留在冻结 payload；5xx、未知码和 in-progress 永不清除原 operation ID。

所有 Sync、Stop、Save、历史 PageScript 与 Playbot 输入必须经过唯一 `RecordingNormalizer`。统一 ActionTrace normalizer 必须拒绝缺失 `type` 的动作，不能让畸形历史脚本进入 Playbot。它生成可持久化 PageScript 视图和脱敏 `recording_source`；Cookie、Storage、密码、token、Authorization、本地路径、二进制和不安全下载数据不得进入 AI。`IsRecoverableRecordingDraft` 额外只接受 Recorder 的 `semantic_dom_snapshot` schema 或显式 `unavailable=true` DOM 标记，任意 JSON 对象不是重启收口依据。`ProjectAuthState` 仅保存 AES-256-GCM 密文、随机 nonce、版本和 key ID；AAD 使用无歧义的 `project-auth-state + state_id + project_id + encryption_version + key_id` 编码。P4.7.6 只有一把活动密钥，不支持在线 keyring/轮换，部署必须执行 `backend/storage/migrations/20260724_p476_project_auth_state_ciphertext.sql`：在事务中先将不完整 runtime identity 的旧活动会话收口为 failed，再回填历史 AutoMigrate 可空字段，避免已经存在的 active partial unique index 因多个 NULL identity 同时归一化而冲突，最后收紧 `NOT NULL` 与 active identity 约束，并清除旧 `state_json` 明文、裁决重复 active 行；应用启动不得静默删除或替代该部署迁移。

敏感输入使用版本化 `SensitiveInputPolicyV1`：Recorder 对密码类型、敏感 autocomplete、以及中英文控件语义提示标记 `sensitive_input=true`，并在录制队列前用 `{{REDACTED_SECRET}}` 覆盖所有值承载字段。Normalizer 必须再次按同一规则检测并强制覆盖，故 PageScript、`recording_source` 和全部内容 hash 只能包含占位符而非原值；回放器不得把该占位符作为真实文本输入，必须由经授权的登录态或 secret source 解析。无效 `data:`/`blob:` 下载动作仅从 ActionTrace 中移除，其他合法动作保留；缺失 `type` 等结构损坏仍拒绝整份录制。可恢复的可用语义 DOM 除 schema/kind 外必须包含非空 URL、字符串标题和 elements 数组。

Stop 的 CAS 重试耗尽时，只能返回带 `Retry-After` 的 `recording_operation_in_progress` 并保留 pending operation、claim 与 receipt；只有 session/operation 已在同一数据库事务中形成终态后才能 ACK 或 release。final receipt 与 auth snapshot 的 ACK/Release 必须同时匹配完整 scope、receipt ID、operation owner 与 `claim_generation`，旧 claim 超时后的 ACK/Release 不得影响接管者。页面内 Stop 只发起确定性 runtime driver request，并复用 `stopBySession` reservation 与锁外 Recorder Stop；Coordinator 是唯一将会话写为 `stopped` 的数据库入口。版本克隆也必须先通过 `NormalizePageScript`：任何历史录制无效时整笔克隆事务回滚并返回脱敏的 `422 recording_source_invalid`，有效副本写入规范化内容 hash/version，且不复制 source session 绑定。

验证：
P4.7.6 正在补跑时序回归（含 HTTP Stop 经 Coordinator `recording_stopped` event 收口、同 operation 重放不二次 runtime Stop）、真实 PostgreSQL 部署迁移升级回归、`go test ./api ./services/browser -count=1`、`go test ./... -count=1`、`pnpm run test:p45-contract`、`pnpm run type-check`、`pnpm run build` 与 `git diff --check`。全部验证通过且部署迁移路径复核完成前，阶段保持修复中；提交不改变该完成条件。
