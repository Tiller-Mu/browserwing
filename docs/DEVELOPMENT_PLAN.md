...
1. 轻量详细设计：明确业务契约、API、数据流、输入输出和验收口径。
2. 用例编写者写契约红测：先证明期望行为，区分回归保护和红测立法。
3. 代码审核者审核红测：确认测试立法合理，避免把偶然实现写成契约。
4. 业务开发者实现：按通过审核的红测修生产代码，追根因，不压表象。
5. 代码审核者复核：审实现是否根因修复，并按影响面跑最终验证。
...
1. 编写 `docs/P1_TEST_CASE_GENERATION_DESIGN.md`，明确生成用例链路的业务契约、API、Playbot 输入输出、保存规则和验收用例。
2. 用例编写者基于 P1 设计写红测，优先覆盖：无主流程拒绝生成、preview 不落库、append 追加、replace 覆盖、Playbot 失败不破坏旧用例。
3. 代码审核者审核红测合理性，确认测试没有固化偶然实现或写第二套事实来源。
4. 业务开发者实现 Playbot Python 路径配置、生成用例 API 和 TestCase 保存逻辑，直到红测转绿。
5. 用例编写者补前端契约或集成用例，覆盖“智能生成测试用例”按钮的主要交互路径。
6. 业务开发者接入前端生成按钮和结果刷新。
7. 代码审核者复核 P1 全链路，运行后端、前端和 Playbot 相关验证。

第二轮：

1. 编写 P2 用例管理详细设计。
2. 用例编写者写 TestCase CRUD、详情读取、编辑保存、删除隔离的契约用例。
3. 代码审核者审核红测。
4. 业务开发者实现后端 TestCase CRUD 和前端用例详情页。
5. 代码审核者复核并跑相关验证。

第三轮：

1. 编写 P3 用例执行详细设计。
2. 用例编写者写 Blueprint 执行、单用例执行 API、执行记录保存的契约用例。
3. 代码审核者审核红测。
4. 业务开发者实现 Blueprint 执行解释器、单用例执行 API、执行记录 API 和前端结果展示。
5. 代码审核者复核并跑相关验证。

第四轮：

1. 编写 P4 自然语言修改详细设计。
2. 用例编写者写 Refinement API、未确认不覆盖、应用后记录历史的契约用例。
3. 代码审核者审核红测。
4. 业务开发者实现 Playbot refine、Refinement API 和前端自然语言修改面板。
5. 代码审核者复核并跑相关验证。

第五轮：

1. 编写 P5 多用户权限详细设计。
2. 用例编写者写项目归属、越权访问、角色权限的契约用例。
3. 代码审核者审核红测。
4. 业务开发者实现项目权限数据模型、API 权限收口和成员管理页面。
5. 代码审核者复核并跑相关验证。
