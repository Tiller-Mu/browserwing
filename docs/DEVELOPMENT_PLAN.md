...
P4.5 已完成。录制体验与项目登录态契约已定稿、红测已通过、实现已复核。下一步先进入 P4.6 PostgreSQL 统一存储迁移；P4.6 后先通过 P4.7/P4.8 收口 LLM、录制数据和端到端生成体验，再进入 P5 多用户和权限规划。
...
P4.6 完成后，不直接进入 P5。下一步先进入 P4.7 LLM 统一配置和录制数据管理，再通过 P4.8 拉通录制到智能生成用例的端到端体验。P5 多用户权限应基于 PostgreSQL 统一存储、Store 边界、LLM 全局配置口径和录制会话数据边界继续设计。
...
1. 规划者定稿 P4.7 设计。
2. 业务开发者 review P4.7 设计可行性，重点反馈 LLM 配置复用、录制会话持久化、浏览器注入同步和录制产物访问边界。
3. 用例编写者先写契约红测，覆盖 LLM 配置选择、禁用/缺失失败、密钥脱敏、RecordingSession 生命周期、草稿动作持久化、保存生成 PageScript、取消失败不污染旧主流程和录制产物元数据。
4. 代码审核者先审红测，确认没有固化当前 `sessionStorage`、裸静态目录或偶然前端路径。
5. 业务开发者只能在红测审核通过后实现生产代码；如发现设计不可行，必须退回规划者补设计。
6. 代码审核者复核实现并运行后端、前端和必要的浏览器录制冒烟验证。
...
1. 规划者编写并定稿 P4.7 LLM 统一配置和录制数据管理详细设计。
2. 业务开发者 review P4.7 设计可行性，重点反馈 LLM 配置复用、录制会话持久化、浏览器注入同步和录制产物访问边界。
3. 用例编写者先写 LLM 配置选择、密钥脱敏、RecordingSession 生命周期、草稿动作持久化、PageScript 生成和录制产物元数据红测。
4. 代码审核者审核红测。
5. 业务开发者实现 LLM 统一解析、录制会话数据模型、录制草稿持久化和录制产物元数据管理。
6. 代码审核者复核并跑相关验证。
7. 规划者更新计划、契约记录和遗留风险。
